...
import { capitalize, classify, dasherize } from '@angular-devkit/core/src/utils/strings';
import {
  apply,
  chain,
  externalSchematic, mergeWith, move, Rule,
  SchematicContext, Source, template, Tree, url
  ...
  return (context: SchematicContext) => {
    ...
    const source = apply(url(join('../common/files' as Path)), [
      template({
        ...
        readme: (name: string) => capitalize(name.replace('-', ' ').replace('  ', ' ')),
        ent: (name: string) => name + '.entity',
        stringify: (name: string) => JSON.stringify(name),
        pluralize,
        conditional: (expr: boolean) => (ifTrue: string) => (otherwise: string) => (expr ? ifTrue : otherwise),
      }),
      move(path),
    ])(context);
    return source;
