...
import { AggregateRootSchema } from './../ddd/aggregate-root/aggregate-root.schema';
import {
  capitalize,
  classify,
  dasherize,
...
function collectImports(aggregateName: string, aggregate: any) {
  return Object.keys(aggregate.props).filter(propName => {
...
export const aggregateRootTemplateRule = (options: AggregateRootSchema) => () => {
  const aggregate = options.model.modules[options.module][classify(options.name)]
  return template({
    classify,
    typeName: classify(options.name),
